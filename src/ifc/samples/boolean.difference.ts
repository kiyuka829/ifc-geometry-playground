...
export const booleanDifferenceSample: SampleDef = {
  ...
  description:
    "An example of boolean operations (IfcBooleanResult). Creates a new shape by combining two solids using a selected operator (DIFFERENCE, UNION, or INTERSECTION).",
  parameters: [
    {
      key: "operator",
      label: "Operator",
  ...
  steps: [
    {
      id: "first",
      label: "Step 1: First Operand",
      description:
        "Define the first solid (the operand). This becomes the base shape.",
    },
    {
      id: "second",
      label: "Step 2: Second Operand (Cutter)",
      description:
        "Display the second solid (the cutter) in semi-transparent. This solid will be used to operate.",
    },
    {
      id: "result",
      label: "Step 3: Boolean Result",
      description:
        "Show the result of the boolean operation. The result depends on the selected operator.",
    },
  ],
  buildGeometry: (
    scene: Scene,
    ...
    return buildBooleanVisualization(
      scene,
      booleanResult,
      "boolean",
      stepIndex,
  ...
  getIFCRepresentation: (params: ParamValues) => {
    const operator: BooleanOperator = getSelect(
      params,
      "operator",
      BOOLEAN_OPERATORS,
      "DIFFERENCE",
