...
export interface IfcBooleanResultNode {
  type: "IfcBooleanResult";
  operator: "DIFFERENCE" | "UNION" | "INTERSECTION";
  firstOperand: IfcExtrudedAreaSolid | IfcBooleanResultNode;
  secondOperand: IfcExtrudedAreaSolid | IfcBooleanResultNode;
}

function buildOperandMesh(
  scene: Scene,
  operand: IfcExtrudedAreaSolid | IfcBooleanResultNode,
  name: string,
): Mesh {
...
export function buildBooleanMesh(
  scene: Scene,
  result: IfcBooleanResultNode,
  name: string,
): Mesh {
  const mat = createSolidMaterial(scene, new Color3(0.3, 0.5, 0.7));
